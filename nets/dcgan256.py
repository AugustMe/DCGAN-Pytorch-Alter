...
class Generator(nn.Module):
    ...
    def __init__(self, nz, ngf, nc):
        ...
        self.net = nn.Sequential(
            # input is Z, going into a convolution（反卷积）
            nn.ConvTranspose2d(nz, ngf * 8, 4, 1, 0, bias=False), # (1-1)*1 + 0 - 2*0 + 4 = 4
            ...
            nn.ConvTranspose2d(ngf, 3, 10, 8, 1, bias=False),
            nn.Tanh() # 输出范围 -1~1 故而采用Tanh
            # state shape: [bs, nc, 256, 256]
        )
...
class Discriminator(nn.Module):
    def __init__(self, ndf, nc):
        ...
        self.net = nn.Sequential(
            # input shape: [bs, nc, 256, 256]
            nn.Conv2d(3, ndf, 10, 8, 1, bias=False),  # (256-10+2)/2 + 1 = 32
